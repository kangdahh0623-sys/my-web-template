        ...
        {plan.length > 0 && (
          <section className="space-y-4">
            ...
            <div className="text-sm text-gray-500 text-center">
              {startIndex + 1} - {Math.min(endIndex, plan.length)}일 표시 중 (전체 {plan.length}일)
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {currentPlan.map((d) => (
                <button
                  key={String(d.day)}
                  onClick={() => setDetail(d)}
                  className="text-left border-2 border-gray-200 rounded-2xl p-4 hover:shadow-xl hover:border-emerald-300 transition-all duration-300 transform hover:scale-105 bg-white"
                >
                  <div className="text-xs font-bold text-emerald-600 mb-2 bg-emerald-50 px-2 py-1 rounded-full text-center">
                    DAY {d.day}
                  </div>
                  ...
                  <div className="space-y-1.5 mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-gray-900 truncate">{d.rice}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">🥣</span>
                      <span className="text-xs text-gray-700 truncate">{d.soup}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">🥬</span>
                      <span className="text-xs text-gray-700 truncate">{d.side1}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">🥕</span>
                      <span className="text-xs text-gray-700 truncate">{d.side2}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">🌶️</span>
                      <span className="text-xs text-gray-700 truncate">{d.side3}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">🍎</span>
                      <span className="text-xs text-blue-600 font-medium truncate">{d.snack}</span>
                    </div>
                  </div>
                  ...
                  <div className="pt-2 border-t border-gray-100">
                    <div className="text-xs text-gray-600 mb-1">
                      <span className="font-semibold text-gray-900">{d.day_kcal.toFixed(0)}</span> kcal
                    </div>
                    <div className="grid grid-cols-3 gap-1 text-xs text-gray-500">
                      <div>C {d.carb_pct_cal.toFixed(0)}%</div>
                      <div>P {d.prot_pct_cal.toFixed(0)}%</div>
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-br from-gray-100 to-gray-200 rounded-3xl p-6 border-4 border-gray-300 shadow-2xl relative">
                    ...
                    {/* 급식판 라벨 */}
                    <div className="absolute top-2 left-2 bg-emerald-600 text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg">
                      DAY {detail.day}
                    </div>
                    
                    {/* 칼로리 표시 */}
                    <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-bold text-gray-700 shadow-lg">
                      {detail.day_kcal.toFixed(0)} kcal
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl p-4 border border-emerald-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white rounded-lg p-3 text-center border border-gray-100">
                        <div className="text-3xl font-bold text-emerald-600">{detail.day_kcal.toFixed(0)}</div>
                        ...
                        <div key={idx} className="space-y-1">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">
                              ...
                              <span className="text-gray-500 ml-1">(목표: {nutrient.target}%)</span>
                            </span>
                          </div>
